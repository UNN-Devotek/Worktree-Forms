...
import { SmartTableColumnEditor } from '../editors/SmartTableColumnEditor'
...
export function PropertiesTab({ field, groupId, formId }: PropertiesTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {field.type === 'smart_table' && (
        <div className="space-y-6 border-t pt-4 mt-4">
            <SmartTableColumnEditor 
                columns={field.columns || []}
                onChange={(columns) => handleUpdate({ columns })}
            />

            <div className="space-y-3">
                 <Label>Table Interactions</Label>
                 <div className="flex items-center justify-between">
                    <Label htmlFor="field-allowAdd" className="font-normal text-sm">Allow Adding Rows</Label>
                    <Switch
                        id="field-allowAdd"
                        checked={field.allowAdd !== false}
                        onCheckedChange={(checked) => handleUpdate({ allowAdd: checked })}
                    />
                 </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="field-allowDelete" className="font-normal text-sm">Allow Deleting Rows</Label>
                    <Switch
                        id="field-allowDelete"
                        checked={field.allowDelete !== false}
                        onCheckedChange={(checked) => handleUpdate({ allowDelete: checked })}
                    />
                 </div>
            </div>
        </div>
