...
3.  **Then** it is resized to 256x256 and saved to S3 (Bucket: `user-profiles`)
...
- [ ] Task 1: Backend API for Avatar
  ...
  - [ ] Save to S3 bucket `user-profiles` with key `{{userId}}/avatar.jpg`.
...
- **Storage**: Use S3 for generic file storage. Ensure `user-profiles` bucket exists in LocalStack/S3.
