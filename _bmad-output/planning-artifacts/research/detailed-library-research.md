...
- **Deployment Frequency**: Ability to deploy to self-hosted production multiple times per day via Git push.
- **Form Builder Utility**: Users can create a "Smart Table" form in < 5 minutes.
- **Offline Reliability**: Field apps sync 100% of data upon reconnection.

## Specific Library Research (Real-Time & PDF)

### Real-Time Spreadsheet Engine

**Selection: FortuneSheet**

- **Rationale**: A mature React wrapper around Luckysheet (MIT Licensed).
- **React Support**: First-class `@fortune-sheet/react` package makes integration into Next.js 14 easier than `Univer` (which is a heavier full-stack framework).
- **Collaboration Pattern**:
  - Use **Yjs** (`y-websocket`) as the source of truth.
  - Create a custom hook `useYjsFortuneSheet` that listens to Yjs `observe` events and calls FortuneSheet's `setCellValue` API.
  - Listen to FortuneSheet's `onAfterChange` event to push updates to Yjs.

### PDF Overlay Engine

**Selection: pdf-lib**

- **Rationale**: The standard Node.js/Browser library for modification.
- **Workflow**:
  1.  **Upload**: Admin uploads PDF -> stored in MinIO.
  2.  **Mapping**: Admin uses a canvas overlay to define X/Y coordinates for each Form Field.
  3.  **Burning**: On export, `pdf-lib` loads the original PDF bytes, draws text at the mapped X/Y coords, and flattens the form.
      _Source_: [pdf-lib.js.org](https://pdf-lib.js.org/)
